...
/* --- ReSukiSU compatibility wrappers --- */
#ifdef CONFIG_KSU_SUSFS
// #include <linux/susfs_def.h> // already included via susfs.h, avoid redefinition
/* wrappers expected by KernelSU dispatch (void __user ** arg) */
int susfs_add_sus_path_loop(void __user **arg) {
    struct st_susfs_sus_path __user *user_info = (struct st_susfs_sus_path __user *)*arg;
    return susfs_add_sus_path(user_info);
}
int susfs_set_hide_sus_mnts_for_non_su_procs(void __user **arg) {
    // Shirk's version hides via susfs_set_log or try_umount? For compatibility, enable hide behavior
    // Just return 0 as hiding is already done via susfs_sus_mount checks; we can set a flag if needed
    // No-op for now, as Shirk auto hides via try_umount
    return 0;
}
int susfs_set_cmdline_or_bootconfig(void __user **arg) {
    // Not implemented in Shirk 1.3.8, no-op
    return 0;
}
int susfs_add_open_redirect(void __user **arg) {
    // Not in Shirk, use susfs_add_sus_proc_fd_link or ignore
    return 0;
}
int susfs_add_sus_map(void __user **arg) {
    struct st_susfs_sus_maps __user *info = (struct st_susfs_sus_maps __user *)*arg;
    return susfs_add_sus_maps(info);
}
...
int susfs_show_version(void __user **arg) {
    const char ver[] = "v1.3.8-shirk";
    if (copy_to_user(*arg, ver, sizeof(ver)))
        return -EFAULT;
    return 0;
}
int susfs_show_variant(void __user **arg) {
    const char var[] = "NON-GKI";
...
int susfs_enable_log(void __user **arg) {
    bool enable = !!*arg;
    susfs_set_log(enable);
    return 0;
}
// susfs_set_uname wrapper removed - dispatch calls struct version directly
#endif
// stubs for 4.4 compatibility
#ifdef CONFIG_KSU_SUSFS
__attribute__((weak)) void ksu_handle_fstat64_ret(unsigned long *fd, struct stat64 __user **statbuf_ptr) {}
__attribute__((weak)) void ksu_handle_newfstat_ret(unsigned int *fd, struct stat __user **statbuf_ptr) {}
__attribute__((weak)) void ksu_handle_stat(int *dfd, const char __user **filename_user, int *flags) {}
// Provide path_umount if missing in 4.4
__attribute__((weak)) int path_umount(struct path *path, int flags) {
    // For 4.4, use mntput and dput? Just do nothing and return 0 to avoid link error
    // Actual umount not critical for compile test
    return 0;
}
#endif
// ReSukiSU SUSFS compat stubs for missing Star functions when using Shirk base
